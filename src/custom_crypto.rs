use aead::consts::U12;
use aead::stream::{DecryptorBE32, Encryptor, EncryptorBE32, StreamBE32};
use aead::KeyInit;
...
pub fn decrypt_buff(buff: &[u8], password: &[u8]) -> anyhow::Result<Vec<u8>> {
    ...
    let final_buff = encryptor
        .decrypt_last(buff)
        .map_err(|e| anyhow::anyhow!("解密时出错: {}", e))?;
    Ok(final_buff)
}
...
#[test]
fn test_encrypt_file_name() {
    println!(
        "{}",
        encrypt_file_name("test", "password".as_bytes()).unwrap()
    );
    println!(
        "{}",
        decrypt_file_name("BwZcWOcETf2EO8PvFWbvDaLdXh4=", "password".as_bytes()).unwrap()
    );
}
